...

{
  ...
  is($constraint1->match_type('foo'), undef, 'Constraint match type rejects bad arg...');
  like($constraint1->error, qr/invalid match type/i, '...because it is invalid');
  is($constraint1->match_type('FULL'), 'full', 'Constraint match type = "full"');
  ...
  like($schema->error, qr/does not exist/i, 'Error on bad arg to get_table');
...
#
{
  ...
  is($c->is_valid,                   undef,   'Constraint on "person" not valid...');
  like($c->error, qr/only one field/i, q[...because too many fields for FK]);

  $c->fields('f1');

  is($c->is_valid, undef, 'Constraint on "person" not valid...');
  like($c->error, qr/no reference table/i, q[...because there's no reference table]);

  is($c->reference_table('foo'), 'foo', 'Reference table now = "foo"');
  is($c->is_valid,               undef, 'Constraint on "person" not valid...');
  like($c->error, qr/no table named/i, q[...because reference table "foo" doesn't exist]);
  ...
  is($c->is_valid, undef, 'Constraint on "person" not valid...');
  like($c->error, qr/no reference fields/i, q[...because there're no reference fields]);

  is(join('', $c->reference_fields('pet_id')), 'pet_id', 'Reference fields = "pet_id"');

  is($c->is_valid, undef, 'Constraint on "person" not valid...');
  like($c->error, qr/non-existent field/i, q[...because there's no "pet_id" field in "pet"]);
...
#
{
  ...
  is($t->primary_key('person_id'), undef, q[Can't make PK on "person_id"...]);
  ...
  is(join(',', $c->fields), 'person_id,name', 'Constraint now on "person_id" and "name"');
...
#
{
  ...
  is($t->perform_action_when,        $perform_action_when, qq[Perform action when is "$perform_action_when"]);
  is(join(',', $t->database_events), $database_events,     qq[Database event is "$database_events"]);
